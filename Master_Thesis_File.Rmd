...
Conversely, the STVP estimation at time $t$ is achieved using a prior of the previous observation at $t-1$. One can observe that the STVP estimation bounds are typically much more narrow than that of the WAR. The posterior distribution that is used involves the error in prediction $e_t$ 
...
$\Omega_t = \sum^{10}_{i=1}w_i(T^{-1}\hat{S_\beta}\kappa_t(c_i) + (\hat{\beta}_{i,t} - \hat{\beta_t})(\hat{\beta}_{i,t} - \hat{\beta_t})')$, $\kappa_t(c) = \frac{c(1 + e^{2c} + e^{2ct/T} + e^2c(1-t/T))}{2e^{2c} - 2}$
