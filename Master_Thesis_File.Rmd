...
Moreover, because we are working with a dataset that has a time element, we can expect the errors of the model above to be autocorrelated. As we haven't yet accounted for the possible variation of $\beta$, the model will be flawed in that it will treat the observations as if they were independent. Previous researchers have used what is called Heteroskedasticity and Autocorrelation-Consistent (HAC) estimators for the variance of our OLS estimator $\beta_1$. We use the Newey-West estimator from 
