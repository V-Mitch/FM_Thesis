...
The equations above are thus the foundation for the construction of the algorithm that is implemented for use and shown in [Section 9.3](#stepsstvp). 
...
## Differences and Irregularities of the Paths
...
On the contrary, the STVP estimation is susceptible to exceptional or isolated changes to the price-reaction to the macro-announcements, especially towards the beginning of the considered sample period. 
...
Conversely, the STVP estimation at time $t$ is achieved using a prior always being the previous observation at $t-1$. One can observe that the STVP estimation bounds are typically much more narrow than that of the WAR. 
Not only does the STVP parameter vary in variance because of the error in prediction $e_t$, but also because the number of observations increases sequentially for every additional step. 
